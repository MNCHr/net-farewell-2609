...
- **전체 표 복사** / **퇴직자별 표 복사** — 각 퇴직자에게 마음을 보탠 사람들에게 메일을 보낼 때
  ...
  > ⚠️ **메일은 반드시 숨은참조(BCC)로 보내세요.** 받는사람·참조에 넣으면 누가 참여했는지
  > 서로에게 다 보입니다. 이 사이트는 "둘 다 체크 안 해도, 한 분만 체크해도 아무도 모른다"는
  > 약속으로 돌아간다 — 명단을 받는사람/참조에 그대로 넣으면 그 약속이 그 자리에서 깨진다.
...
1. **구글시트**
   - `responses` A1 을 `empNo` → `email` 로 고친다
   - **사번 시절 테스트 행을 전부 지운다** — 지우지 않으면 `01629@etri.re.kr` 같은 가짜 주소가
     명단과 메일 수신자 목록에 섞인다. 이런 행은 경고 없이 조용히 섞여 들어가고, 나중에
     메일이 반송될 때에야 드러난다
   - `log` 시트도 헤더 1행만 남기고 비운다
...
4. **확인** — `test.html` 을 열어 `headerOk` 가 OK 인지 본다. 1번을 빠뜨렸으면 여기서 잡힌다.
   실패하면 원인은 하나가 아니라 둘이다: **헤더가 다르거나, Apps Script 를 새 버전으로
   배포하지 않았습니다.** 시트가 맞다면 재배포(2번)를 다시 확인하세요 — 특히 2번을
   빠뜨리고 이 페이지부터 열어보는 경우가 가장 흔하다
